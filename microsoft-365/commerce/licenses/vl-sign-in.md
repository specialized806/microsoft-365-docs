...
> [!NOTE]
> Microsoft 365 admin center global administrators don't have access to VL features in the Microsoft 365 admin center. It's up to the VL agreement administrator to add any users, including global administrators, who might want access to specific license IDs. For more information, see [Commonly used Microsoft 365 admin center roles](../../admin/add-users/about-admin-roles.md#volume-licensing-roles), and [Volume licensing roles](../../admin/add-users/about-admin-roles.md#volume-licensing-roles).
...
> [!NOTE]
> Check your junk or spam folders in case you can’t find the email in your inbox.
...
Organizations with no online presence with Microsoft must [Sign Up](https://signup.microsoft.com/get-started/signup?products=8339cc50-d965-4ad5-bb94-749021a5ebf9&origin=tenantonly&ali=1&ru=https%3a%2f%2fadmin.microsoft.com) for a new Microsoft Entra account (also known as a tenant) to allow VL users access to the Microsoft 365 admin center. This doesn't require the user, or the organization, to buy or use any Microsoft products or services, like Microsoft 365.

To create a Microsoft Entra ID using your organization's email domain (like user\@fourthcoffeexyz.com), you must provide proof that the organization owns that domain. This proof might be something that your organization’s IT administrator prefers to complete, rather than the VL user.

Alternatively, if you need a Microsoft Entra ID format that isn't linked to the organization’s email domain (like user\@fourthcoffeexyz.onmicrosoft.com), you can create one independently from an IT administrator. Before you deploy any online services like Microsoft 365, we highly recommend that your organization takes over such a Microsoft Entra ID tenant and link it to the organization's own domain.
...
- A Microsoft Entra ID that's different from the one that you used previously to sign in for volume licensing.  For example, you might be trying to sign in with your work email user\@contoso.com whereas you previously signed in to volume licensing with licenseAdmin\@contoso.com.
...
After your organization converts your account to a managed Microsoft Entra ID, or you identify a suitable alternative Microsoft Entra ID, contact Microsoft Volume Licensing Support to request a new volume licensing profile. When you contact support, you must provide the following items:
