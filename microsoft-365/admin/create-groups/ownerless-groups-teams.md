...
- **Member can't see *Yes* and *No* in the notification email.**
  ...
  - Make sure that the user principal name (UPN) and primary email address of the sender are the same. For information about how to update the UPN or primary email address of a user, see [Change a user name and email address](../../change-a-user-name-and-email-address.md). If they don't match, the recipient can't see the **Yes** and **No** options.
